  ...
  useEffect(() => {
    async function loadItems() {
      ...
      const nextItems = await Promise.all((links || []).map(async (link) => {
        ...
        try { const { data: live } = await client.queries.queryApass({ workspaceId, workspaceIdentityId: link.id }); countries = (live?.countries || []).filter((country): country is string => Boolean(country)); } catch { /* Detail page will show the live error. */ }
        return { ...wallet, ...link, walletAddress: wallet?.walletAddress, chain: wallet?.chain, countries };
      }));
      setItems(nextItems);
    }
    loadItems();
