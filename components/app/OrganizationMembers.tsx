        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            ...
            {!loading && members.map((member) => {
              ...
              const friendlyName = isAdmin ? "Workspace owner" : member.displayName && !isUserId(member.displayName) ? member.displayName : "Pending member";
              const avatarText = isAdmin ? "WO" : friendlyName === "Pending member" ? "PM" : initials(friendlyName);
              return <tr key={member.id} className="border-b border-hair last:border-0">
                <td className="px-5 py-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo/10 text-xs font-semibold text-indigo">{avatarText}</span><div><p className="font-medium text-ink">{friendlyName}</p><p className="mt-1 text-xs text-sub">ID {member.userId.slice(0, 8)}...</p></div></div></td>
