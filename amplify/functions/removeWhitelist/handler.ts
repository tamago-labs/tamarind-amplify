...
import { env } from "$amplify/env/removeWhitelist";
...
export async function handler(
  ...
  try {
    ...
    if (removeReason) {
      body.removeReason = removeReason;
    }

    const url = `${BASE_URL}/atoken/remove_whitelist_for_institutional`;
    ...
    if (json.code === "0000" || json.code === "0" || json.code === "200") {
      ...
      return {
        success: false,
        error: json.message || "Failed to remove from whitelist",
      };
    }
  } catch (error) {
